...
const Heading = styled.div`text-align:center;margin:0 0 3rem; h2{margin:0 0 1rem;font-size:2.4rem;color:${p=>p.theme.colors.primary};} p{margin:0 auto;max-width:640px;opacity:.85;}`;
const Grid = styled.div`display:grid;gap:2rem;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));`;
const Card = styled.div`position:relative;background:rgba(255,255,255,.03);border:1px solid rgba(212,175,55,.12);padding:1.75rem 1.5rem;border-radius:14px;display:flex;flex-direction:column;gap:1rem;transition:.3s; &:hover{transform:translateY(-6px);border-color:${p=>p.theme.colors.primary};}`;
...
const Body = styled.p`font-style:italic;line-height:1.55;font-size:.95rem;margin:0;`;
const Row = styled.div`display:flex;align-items:center;gap:.85rem;`;
const Avatar = styled.div`width:54px;height:54px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid ${p=>p.theme.colors.primary};background:rgba(212,175,55,0.08);color:${p=>p.theme.colors.primary};`;
const Meta = styled.div`h4{margin:0 0 .25rem;font-size:.95rem;color:${p=>p.theme.colors.primary};} span{display:block;font-size:.65rem;text-transform:uppercase;opacity:.5;}`;
